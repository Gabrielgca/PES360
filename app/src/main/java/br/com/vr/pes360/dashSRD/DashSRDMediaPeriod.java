 ...
 */
package br.com.vr.pes360.dashSRD;
...
import com.google.android.exoplayer2.source.chunk.OurChunkSampleStream.EmbeddedSampleStream;
...
import br.com.vr.pes360.tracking.ReplacementTracker;
import br.com.vr.pes360.tracking.TileQualityTracker;
//ESTUDAR USO
...
/* package */ final class DashSRDMediaPeriod
        implements MediaPeriod,
        SequenceableLoader.Callback<OurChunkSampleStream<DashChunkSource>>,
        OurChunkSampleStream.ReleaseCallback<DashChunkSource> {
    ...
    private final IdentityHashMap<OurChunkSampleStream<DashChunkSource>, PlayerTrackEmsgHandler>
    ...
    private OurEventSampleStream[] OurEventSampleStreams;
    ...
    public DashSRDMediaPeriod(
            ...
            TileQualityTracker tileQualityTracker,
        ...
        OurEventSampleStreams = new OurEventSampleStream[0];
...
//        trackGroups = buildTrackGroups(period.adaptationSets);
    }
    ...
    public void updateManifest(DashManifest manifest, int periodIndex) {
        ...
        for (OurEventSampleStream OurEventSampleStream : OurEventSampleStreams) {
            for (EventStream eventStream : eventStreams) {
                if (eventStream.id().equals(OurEventSampleStream.eventStreamId())) {
                    int lastPeriodIndex = manifest.getPeriodCount() - 1;
                    OurEventSampleStream.updateEventStream(
                            eventStream,
    ...
    // OurChunkSampleStream.ReleaseCallback implementation.

    @Override
    public synchronized void onSampleStreamReleased(OurChunkSampleStream<DashChunkSource> stream) {
    ...
    public long selectTracks(
        ...
        ArrayList<OurChunkSampleStream<DashChunkSource>> sampleStreamList = new ArrayList<>();
        ArrayList<OurEventSampleStream> OurEventSampleStreamList = new ArrayList<>();
        for (SampleStream sampleStream : streams) {
            if (sampleStream instanceof OurChunkSampleStream) {
                @SuppressWarnings("unchecked")
                OurChunkSampleStream<DashChunkSource> stream =
                        (OurChunkSampleStream<DashChunkSource>) sampleStream;
                sampleStreamList.add(stream);
            } else if (sampleStream instanceof OurEventSampleStream) {
                OurEventSampleStreamList.add((OurEventSampleStream) sampleStream);
            }
        }
        sampleStreams = newSampleStreamArray(sampleStreamList.size());
        sampleStreamList.toArray(sampleStreams);
        OurEventSampleStreams = new OurEventSampleStream[OurEventSampleStreamList.size()];
        OurEventSampleStreamList.toArray(OurEventSampleStreams);
    ...
    public long seekToUs(long positionUs) {
        ...
        for (OurEventSampleStream sampleStream : OurEventSampleStreams) {
            sampleStream.seekToUs(positionUs);
        }
        return positionUs;
    }
        ...
        for (int i = 0; i < selections.length; i++) {
            if (selections[i] == null || !mayRetainStreamFlags[i]) {
                if (streams[i] instanceof OurChunkSampleStream) {
                    @SuppressWarnings("unchecked")
                    OurChunkSampleStream<DashChunkSource> stream =
                            (OurChunkSampleStream<DashChunkSource>) streams[i];
                    stream.release(this);
        ...
        for (int i = 0; i < selections.length; i++) {
            ...
            if (streams[i] == null) {
                ...
                if (trackGroupInfo.trackGroupCategory == TrackGroupInfo.CATEGORY_PRIMARY) {
                    ...
                    streams[i] = new OurEventSampleStream(eventStream, format, manifest.dynamic);
                }
            } else if (streams[i] instanceof OurChunkSampleStream) {
            ...
            if (streams[i] == null && selections[i] != null) {
                ...
                if (trackGroupInfo.trackGroupCategory == TrackGroupInfo.CATEGORY_EMBEDDED) {
                    ...
                    if (primaryStreamIndex == C.INDEX_UNSET) {
                        ...
                        streams[i] =
                                ((OurChunkSampleStream) streams[primaryStreamIndex])
    ...
    private OurChunkSampleStream<DashChunkSource> buildSampleStream(
            TrackGroupInfo trackGroupInfo, ExoTrackSelection selection, long positionUs) {
        //Verificar a exclusão de toda a parte relacionada a EmbeddedTracks, que não era utilizada no TOUCAN
