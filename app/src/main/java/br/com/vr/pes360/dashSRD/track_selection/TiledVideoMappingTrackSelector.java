...
/**
 * A {@link TrackSelector} class based on MappingTrackSelector. It first
 * establishes a mapping between {@link TrackGroup}s and renderers, and then
 * from that mapping create a {@link TrackSelection} for each renderer.
 *
 * The class is extended to handle the case in which there are multiple video
 * renderers and each should be exposed to a different TrackGroup.
 *
 */
public abstract class TiledVideoMappingTrackSelector extends TrackSelector {
  ...
  /**
   * Implementation of the TrackSelector. It maps TrackGroups to renderers, and
   * then it creates a {@link TrackSelection} for each renderer.
   *
   * N.B. modifications: introduced selectedIndexes to keep track of the renderers
   * already assigned and avoid having the same video renderer assigned to all the
   * video track groups.
   */
  ...
  public final TrackSelectorResult selectTracks(
    ...
    for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
      ...
      int rendererIndex =
              findRenderer(
                      rendererCapabilities, group, rendererTrackGroupCounts, preferUnassociatedRenderer, selectedIndexes);
  ...
  private static int findRenderer(
    ...
    for (int rendererIndex = 0; rendererIndex < rendererCapabilities.length; rendererIndex++) {
      if (!selectedIndexes[rendererIndex]) {
        ...
        if (formatSupportLevel > bestFormatSupportLevel
          ...
          if (!selectedIndexes[bestRendererIndex]) {
            selectedIndexes[bestRendererIndex] = true;
